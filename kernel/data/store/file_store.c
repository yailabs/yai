...
int yai_data_store_init_scope(const char *data_scope_id, char *err, size_t err_cap)
{
  ...
  if (yai_data_store_binding_attach_scope(data_scope_id, err, err_cap) != 0) return -1;
  if (yai_data_store_paths(data_scope_id, "events", dir, sizeof(dir), NULL, 0, path_err, sizeof(path_err)) != 0) {
                          ...
                          size_t err_cap)
{
  ...
  if (yai_data_store_init_scope(workspace_id, err, err_cap) != 0) return -1;
