 ...
 *
 * Return: YAI IRQ number on success or error code on failure
 */
			     ...
			     const struct cpumask *dest)
{
	...
	if (irq_domain_is_ipi_single(domain)) {
		 ...
		 * all cpus then we only need a single YAI irq number for
 ...
 * @irq:	YAI IRQ number to be destroyed
...
/**
 * __ipi_send_single - send an IPI to a target YAI SMP CPU
...
#ifdef DEBUG
	/*
	 * Minimise the overhead by omitting the checks for YAI SMP IPIs.
...
/**
 * __ipi_send_mask - send an IPI to target YAI SMP CPU(s)
 * @desc:	pointer to irq_desc of the IRQ
 ...
 * @virq:	YAI IRQ number from irq_reserve_ipi()
 * @cpu:	destination CPU, must in the destination mask passed to
 *		irq_reserve_ipi()
 *
 * Return: %0 on success or negative error number on failure.
