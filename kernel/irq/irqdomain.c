 ...
 * @type:	Type of irqchip_fwnode. See yai/irqdomain.h
						   ...
						   bool cond_alloc_descs, bool force_associate)
{
	...
	/*
	 * Legacy interrupt domains have a fixed YAI interrupt number
	 * associated. Other interrupt domains can request association by
	 * providing a YAI interrupt number > 0.
	 */
 ...
 * the yai irq as the hardware interrupt number. It still uses the linear
...
/**
 * irq_create_mapping_affinity() - Map a hardware interrupt into yai irq space
 * @domain: domain owning this hardware interrupt or NULL for default domain
 * @hwirq: hardware irq number in that domain space
 * @affinity: irq affinity
 *
 * Only one mapping per hardware interrupt is permitted. Returns a yai
 * irq number.
 ...
 * @virq: yai irq number of the interrupt to unmap
 */
...
/**
 * __irq_resolve_mapping() - Find a yai irq from a hw irq number.
 * @domain: domain owning this hardware interrupt
 * @hwirq: hardware irq number in that domain space
 * @irq: optional pointer to return the YAI irq if required
 *
