...
pub static APPLICATION_OPERATIONS: &[ApplicationOperationDescriptor] = &[
    ...
    operation("execution.get", "Observe an exact durable submission without redispatch; admitted participant Resource requests expose exact continuation input, retained process exit facts and optional output requalify current authority, and optional Conversation context inspection requalifies archived W under current disclosure", "yai.execution_get_input.v1", "yai.execution_observation.v1", StateImpact::Read, AuthorityClass::CurrentCaseAuthority),
    operation("execution.list", "Discover bounded recent execution references under each current owner disclosure check; no outputs or dispatch", "yai.execution_list_input.v1", "yai.execution_list_projection.v1", StateImpact::Read, AuthorityClass::CurrentCaseAuthority),
    ...
    operation("handoff.inspect", "Inspect one visible Handoff and current Case-local protocol facts without transferring authority", "yai.handoff_inspect_input.v1", "yai.handoff_inspection.v1", StateImpact::Read, AuthorityClass::CaseDisclosure),
    ...
    operation("handoff.pending", "List pending incoming offers from the existing bounded authorized Tenant Case scan", "yai.handoff_pending_input.v1", "yai.handoff_pending_projection.v1", StateImpact::Read, AuthorityClass::CaseDisclosure),
    ...
    operation("machine.get", "Inspect one exact Tenant machine pin and revocation under current membership", "yai.machine_get_input.v1", "yai.machine_asset_view.v1", StateImpact::Read, AuthorityClass::LocalAuthenticated),
    operation("machine.list", "List only the authenticated Principal's Tenant machine registrations", "yai.machine_list_input.v1", "yai.machine_asset_view_list.v1", StateImpact::Read, AuthorityClass::LocalAuthenticated),
    operation("machine.register", "Register one independently approved exact SSH host identity without probing or executing", "yai.machine_register_input.v1", "yai.machine_asset_view.v1", StateImpact::CanonicalMutation, AuthorityClass::TenantOwner),
    operation("machine.revoke", "Revoke one exact Tenant machine registration without changing Case or remote host state", "yai.machine_revoke_input.v1", "yai.machine_asset_view.v1", StateImpact::CanonicalMutation, AuthorityClass::TenantOwner),
    ...
    operation("provider.models", "Discover exact exposed models; an authorized compatibility target may project exact public catalog capacity, without qualification or residency claims", "yai.provider_models_input.v1", "yai.provider_models.v1", StateImpact::DerivedComputation, AuthorityClass::CurrentCaseAuthority),
    operation("provider.probe", "Owner-authorized synthetic probe with exact durable identity; no Case material, trust or binding", "yai.provider_probe_input.v1", "yai.provider_probe_execution.v1", StateImpact::ExternalEffect, AuthorityClass::LocalAuthenticated),
    operation("provider.probe.get", "Observe an exact retained synthetic probe under current Tenant owner authority without dispatch", "yai.provider_probe_get_input.v1", "yai.provider_probe_execution.v1", StateImpact::Read, AuthorityClass::LocalAuthenticated),
    operation("provider.probe.list", "Discover retained target probe identities under current Tenant Owner authority without dispatch", "yai.provider_probe_list_input.v1", "yai.provider_probe_list.v1", StateImpact::Read, AuthorityClass::LocalAuthenticated),
    ...
    operation("resource.import", "Resolve an explicit Resource definition under current owner authority and attach its immutable native configuration; exact retry does not duplicate attachment", "yai.resource_import_input.v1", "yai.case_state_mutation_result.v1", StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority),
...
pub static CAPABILITIES: &[ProductCapabilityDescriptor] = &[
    ...
    capability("case.lifecycle", "Create, open, run, resume, stop, cancel and close governed Cases", "Case transition/controller owners", &["K03", "X01"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["case.cancel", "case.close", "case.create", "case.open", "case.resume", "case.run", "case.stop", "execution.get"], None, CliPosture::Exposed, &["yai.case.create", "yai.case.run", "yai.case.resume", "yai.case.stop", "yai.case.cancel", "yai.case.close"], None, StudioPosture::UiAlreadyConsumed, None, Some("Studio consumes create/open/cancel/close, bounded run/stop/observation and exact operator-stopped checkpoint continuation. Other resumable checkpoint postures remain presentation debt; YAI owns resume admission.")),
    ...
    capability("case.workspace", "Project current Case overview, environment, authority, work, compute and conversation", "yai-application projection over engine owners", &["X03", "X04"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CaseDisclosure, ApplicationPosture::Ready, &["case.open", "case.summary", "events.heartbeat", "events.subscribe", "events.resume", "execution.list"], None, CliPosture::Exposed, &["yai.case.show", "yai.case.executions"], None, StudioPosture::UiAlreadyConsumed, None, Some("Case summary and bounded recent execution reference discovery are consumed. Work/Executions select an exact current-authorized observation; complete history remains outside this bounded projection.")),
    ...
    capability("cognitive.decision_trajectory", "Reconstruct disclosed historical Decision prerequisites and qualified consequences; export and evaluate a bounded structured corpus without scoring choices", "historical semantic reader + exact experience relations", &["E07", "Q06"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CaseDisclosure, ApplicationPosture::Ready, &["decision.trajectory.corpus", "decision.trajectory.evaluate", "decision.trajectory.inspect"], None, CliPosture::Exposed, &["yai.case.trajectory", "yai.case.trajectory.corpus", "yai.case.trajectory.evaluate"], None, StudioPosture::UiAlreadyConsumed, None, None),
    ...
    capability("conversation.execution", "Draft/send Turns and execute ordinary Recall-aware Conversation work", "shared cognitive execution + Turn/intent/Invocation owners", &["C06", "E03"], CapabilityDisposition::ProductAction, StateImpact::ExternalEffect, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["conversation.send", "execution.get"], None, CliPosture::Exposed, &["yai.case.conversation.draft.send", "yai.case.workbench"], None, StudioPosture::UiAlreadyConsumed, None, Some("Studio text SEND, exact retry, read-only result recovery and immutable focused-context selection are qualified; focused also requests a 1024-token maximum output in exact preflight and inference. The interactive CLI offers the same focused context through /context. The Output panel observes exact retained Conversation responses without redispatch. Attachment authoring and live token streaming remain separate.")),
    capability("conversation.inspect", "Inspect authorized retained Conversation turns", "Turn/Conversation projection", &["K02", "X03"], CapabilityDisposition::ProductRead, StateImpact::Read, AuthorityClass::CaseDisclosure, ApplicationPosture::Ready, &["case.summary"], None, CliPosture::Exposed, &["yai.case.conversation.turn.list", "yai.case.conversation.turn.show"], None, StudioPosture::UiAlreadyConsumed, None, Some("Conversation summary includes canonical part_ref identities beside disclosed content; identities select existing material and confer no execution authority.")),
    capability("effect.controlled_execution", "Submit governed Resource/effect requests through Decision, review, PREPARE and final fences", "effect admission + carrier", &["A03", "A05", "O07"], CapabilityDisposition::ProductAction, StateImpact::ExternalEffect, AuthorityClass::ReviewGrantAndEffectFence, ApplicationPosture::Ready, &["effect.propose", "effect.reconcile", "effect.submit", "execution.get", "resource.request"], None, CliPosture::Exposed, &["yai.case.resource.request", "yai.case.resource.observe", "yai.effect.filesystem_write", "yai.effect.process_signal", "yai.effect.reconcile"], None, StudioPosture::ApplicationReady, None, Some("Studio consumes Resource requests, exact reviewed process-request continuation after Host restart with stale confirmation and lost-ACK receipt recovery, and explicit retained-result filesystem/process proposal and submission, with lost-ACK recovery and Review refusal tests. Historical candidate lineage resolves from the canonical Case prefix at commit/replay without replacing the latest-result projection. Studio also authors exact reconciliation with observation-only default and explicit filesystem no-effect recovery; lost-ACK receipt identity and stale/hidden refusal are qualified. Process reconciliation interaction remains unqualified. Authorized process status and opt-in retained stdout/stderr are consumed via execution.get with exact bytes, nonzero exit, restart/retry and revocation tests. CLI Resource observe uses the same typed execution.get dispatcher; --output explicitly reads retained process output without dispatch.")),
    capability("handoff.lifecycle", "Offer, accept, decline, result and reconcile explicit same-Tenant Handoffs", "Handoff transitions", &["W02"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["handoff.accept", "handoff.decline", "handoff.inspect", "handoff.offer", "handoff.pending", "handoff.reconcile", "handoff.result.record"], None, CliPosture::Exposed, &["yai.case.handoff.offer", "yai.case.handoff.pending", "yai.case.handoff.show", "yai.case.handoff.accept", "yai.case.handoff.decline", "yai.case.handoff.result", "yai.case.handoff.reconcile"], None, StudioPosture::UiAlreadyConsumed, None, Some("Authored mutations and bounded incoming inbox/exact inspection are consumed. Complete outgoing/history catalog remains outside the current read projection.")),
    ...
    capability("machine.asset_registry", "Register and revoke exact Tenant machine SSH identities independently of provider targets", "Tenant machine asset owner", &["X03"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::TenantOwner, ApplicationPosture::Ready, &["machine.get", "machine.list", "machine.register", "machine.revoke"], None, CliPosture::Exposed, &["yai.machine.get", "yai.machine.list", "yai.machine.register", "yai.machine.revoke"], None, StudioPosture::UiAlreadyConsumed, None, Some("Studio Machines consumes exact list/get/register/revoke through the Host with invalid-key rejection and Case-isolation proof. Registration stores an independently attested host-key pin, not network reachability or YVEX runtime management. Remote status and lifecycle require a qualified public transport consumer.")),
    ...
    capability("provider.governance", "Register, qualify, trust and bind generic provider targets", "provider governance", &["E01", "E02"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["provider.case.bind", "provider.probe", "provider.probe.get", "provider.probe.list", "provider.qualify", "provider.register", "provider.suitability.record", "provider.trust.set"], None, CliPosture::Exposed, &["yai.provider.add", "yai.provider.probe", "yai.provider.probes", "yai.provider.qualify", "yai.provider.trust.approve", "yai.case.provider.bind"], None, StudioPosture::ApplicationReady, None, Some("Studio consumes target registration, mechanical evidence, trust and Case binding; primary-conversation operator attestation is integrated; other cognitive-capability attestation authors remain unintegrated.")),
    capability("provider.inspect", "Inspect Tenant inventory and Case-bound provider targets with current governed posture", "provider governance projection", &["E01", "X03"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CaseDisclosure, ApplicationPosture::Ready, &["case.summary", "provider.models", "provider.inventory"], None, CliPosture::Exposed, &["yai.provider.list", "yai.provider.show", "yai.provider.models", "yai.case.provider.show"], None, StudioPosture::UiAlreadyConsumed, None, None),
    ...
    capability("resource.lifecycle", "Attach and inspect exact governed filesystem/process/other Resources", "ResourceAttachment transitions", &["O01", "O03", "O07"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["case.summary", "resource.attach", "resource.attach_process", "resource.import"], None, CliPosture::Exposed, &["yai.case.resource.attach_filesystem", "yai.case.resource.attach_process", "yai.case.resource.import", "yai.case.resource.list"], None, StudioPosture::ApplicationReady, None, Some("Typed Resource import resolves native configuration through the shared CLI owner. Studio authors process attachment and bounded filesystem/discovery, SQLite query, HTTP, MCP catalog and exact-digest process-runner setup with retry/refusal proof. Arbitrary low-level attachment, runner environment authoring and reconnect-safe continuation after Review remain Studio debt. Missing PID is an action refusal, not transport loss.")),
    ...
    capability("semantic.fast_search", "Prepare optional System-1 memory navigation over exact current Recall/W choices with deterministic fallback", "Cognitive Decision Plane + Recall/W", &["E07", "M06"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["semantic.fast_search.prepare"], None, CliPosture::Exposed, &["yai.case.cognitive.fast_search"], None, StudioPosture::UiAlreadyConsumed, None, Some("Working State inspects exact navigation choices and omissions with stale refusal. No public production Decision Readout exists; preparation reports unavailable and never scores. Conversation fast preference falls back to standard.")),
    ...
    capability("source.lifecycle", "Declare, acquire, resume, publish and revoke Case-local source relations", "source acquisition + Case source transitions", &["O08"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["execution.get", "source.acquire", "source.declare", "source.publish", "source.resume", "source.revoke"], None, CliPosture::Exposed, &["yai.case.sources.declare", "yai.case.sources.acquire", "yai.case.sources.resume", "yai.case.sources.publish", "yai.case.sources.revoke"], None, StudioPosture::UiAlreadyConsumed, None, Some("Studio consumes declare/acquire/resume/publish/revoke and exact attempts. Real HTTP Source interruption is qualified through the Host and authored UI: running becomes delivery_indeterminate after carrier loss; refresh never redispatches, acquire/resume remain disabled, exact retry preserves identity, hidden observation is refused. Review-approved continuation is qualified across Host restart, stale confirmation and lost acknowledgement: approval alone never fetches; explicit resume retains the attempt and completed exact backing. Retained material uses the shared current Review/policy disclosure gate, and Studio requalifies clean content on same-generation snapshot refresh. Indeterminate delivery still cannot resume automatically.")),
