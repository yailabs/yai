...
pub static CAPABILITIES: &[ProductCapabilityDescriptor] = &[
    ...
    capability("provider.inspect", "Inspect Case-bound provider targets and current governed posture", "provider governance projection", &["E01", "X03"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CaseDisclosure, ApplicationPosture::Ready, &["case.summary", "provider.models"], None, CliPosture::Exposed, &["yai.provider.list", "yai.provider.show", "yai.case.provider.show"], None, StudioPosture::UiAlreadyConsumed, None, None),
...
|---|---|---|---|---|---|\n",
    ...
    if APPLICATION_BLOCKERS.is_empty() {
        output.push_str("No retained Application execution-lifecycle blocker among the 31 eligible PRODUCT families. Host lifecycle is the carrier-level exception, not an Application operation. Ready does not imply automatic retry, universal recovery or Studio interaction coverage.\n");
    } else {
