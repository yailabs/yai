...
pub static CAPABILITIES: &[ProductCapabilityDescriptor] = &[
    ...
    capability("conversation.execution", "Draft/send Turns and execute ordinary Recall-aware Conversation work", "shared cognitive execution + Turn/intent/Invocation owners", &["C06", "E03"], CapabilityDisposition::ProductAction, StateImpact::ExternalEffect, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["conversation.send", "execution.get"], None, CliPosture::Exposed, &["yai.case.conversation.draft.send", "yai.case.workbench"], None, StudioPosture::UiAlreadyConsumed, None, Some("Studio text SEND, exact retry, read-only result recovery and immutable focused-context selection are qualified; the interactive CLI offers the same focused context through /context. The Output panel observes exact retained Conversation responses without redispatch. Attachment authoring and live token streaming remain separate.")),
