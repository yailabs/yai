...
const CASE_RUNTIME_BLOCKED: Option<&str> = Some("Create/open/cancel/close are typed Application operations. Run/resume/stop remain blocked on a resident RuntimeInstance supervision lifecycle; a Host request cannot safely inherit CLI-process checkpoint ownership.");
const EXECUTION_LIFECYCLE_BLOCKED: Option<&str> = Some("The non-executing typed operations are exposed. Provider/resource dispatch remains blocked on a disconnect-safe, idempotent Application execution lifecycle; transport loss cannot be mapped safely to retry or completion from the current process-local controller contract.");
...
pub static APPLICATION_BLOCKERS: &[ApplicationCapabilityBlocker] = &[
    ApplicationCapabilityBlocker {
        ...
        existing_partial_operation_ids: &["case.cancel", "case.close", "case.create", "case.open"],
    },
    ApplicationCapabilityBlocker {
    ...
    ApplicationCapabilityBlocker {
        ...
        class: ApplicationBlockerClass::DisconnectSafeExecutionLifecycle,
        missing_contract: "Typed acquisition attempt identity and result observation across governed Resource dispatch, review and reconnect; declare/publish/revoke are already exposed.",
        existing_partial_operation_ids: &["source.declare", "source.publish", "source.revoke"],
    },
];
...
pub static CAPABILITIES: &[ProductCapabilityDescriptor] = &[
    ...
    capability("effect.controlled_execution", "Submit governed Resource/effect requests through Decision, review, PREPARE and final fences", "effect admission + carrier", &["A03", "A05", "O07"], CapabilityDisposition::ProductAction, StateImpact::ExternalEffect, AuthorityClass::ReviewGrantAndEffectFence, ApplicationPosture::Deferred, &[], EXECUTION_LIFECYCLE_BLOCKED, CliPosture::Exposed, &["yai.case.resource.request", "yai.effect.filesystem_write", "yai.effect.process_signal", "yai.effect.reconcile"], None, StudioPosture::UiTarget, None, None),
    capability("handoff.lifecycle", "Offer, accept, decline, result and reconcile explicit same-Tenant Handoffs", "Handoff transitions", &["W02"], CapabilityDisposition::ProductAction, StateImpact::CanonicalMutation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["handoff.accept", "handoff.decline", "handoff.offer", "handoff.reconcile", "handoff.result.record"], None, CliPosture::Exposed, &["yai.case.handoff.offer", "yai.case.handoff.accept", "yai.case.handoff.decline", "yai.case.handoff.result", "yai.case.handoff.reconcile"], None, StudioPosture::ApplicationReady, None, None),
    ...
    capability("semantic.recall", "Reconstruct task/query-qualified D/H/S evidence with exact closure and disclosure", "Recall v2", &["M06", "Q06"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CaseDisclosure, ApplicationPosture::Ready, &["semantic.recall"], None, CliPosture::Exposed, &["yai.case.recall"], None, StudioPosture::ApplicationReady, None, None),
    ...
    capability("semantic.working_state", "Compile W3, exact-group W4 pages and explicit same-task refresh", "SemanticWorkingState compiler/refresh/paging", &["C02", "C06", "C07"], CapabilityDisposition::ProductRead, StateImpact::DerivedComputation, AuthorityClass::CurrentCaseAuthority, ApplicationPosture::Ready, &["semantic.working_state.compile", "semantic.working_state.page", "semantic.working_state.refresh"], None, CliPosture::Exposed, &["yai.case.context.compile", "yai.case.context.refresh", "yai.case.context.expand"], None, StudioPosture::ApplicationReady, None, None),
