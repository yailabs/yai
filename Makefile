...
CHARACTERIZATION_PROVIDER_GOVERNANCE_HARDENING := tests/characterization/provider-governance-hardening/test_provider_governance_hardening.sh
CHARACTERIZATION_MEMORY_REPRESENTATION := tests/characterization/memory-representation/test_memory_representation.sh
CHARACTERIZATION_MEMORY_INDEX_HARDENING := tests/characterization/memory-index-hardening/test_memory_index_hardening.sh
CHARACTERIZATION_EPISODIC_SEMANTIC_MEMORY := tests/characterization/episodic-semantic-memory/test_episodic_semantic_memory.sh
CHARACTERIZATION_MULTIPART_CONVERSATION := tests/characterization/multipart-conversation/test_multipart_conversation.sh
CHARACTERIZATION_COGNITIVE_EXECUTION_COMPOSITION := tests/characterization/cognitive-execution-composition/test_cognitive_execution_composition.sh
...
info:
	...
	@printf "memory-indexing: content-addressed documents + derived BM25/exact-cosine/RRF under YAI_HOME; ANN deferred\n"
...
smoke-recall-working-state: build-rust
	@python3 tools/validation/topology.py label --entry $@
	@python3 tests/characterization/source-grounded-knowledge/test_working_state.py
...
.PHONY: smoke-cross-case-source-reuse
smoke-cross-case-source-reuse: build-rust
	@python3 tools/validation/topology.py label --entry $@
...
smoke-recall-trace: build-rust
	@python3 tools/validation/topology.py label --entry $@
	@python3 tests/characterization/recall-trace/test_recall.py
...
.PHONY: smoke-provider-connection qualification-provider-connection
smoke-provider-connection: build-rust
	@python3 tools/validation/topology.py label --entry $@
	@python3 tests/characterization/cli-product-surface/test_provider_connection.py

qualification-provider-connection: build-rust
	@python3 tools/validation/topology.py label --entry $@
	@python3 tests/characterization/cli-product-surface/test_provider_connection.py --external
