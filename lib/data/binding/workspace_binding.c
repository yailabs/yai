...
int yai_data_store_binding_attach_workspace(const char *workspace_id,
                                            char *err,
                                            size_t err_cap)
{
  char ws_root[768];
  ...
  if (snprintf(ws_root, sizeof(ws_root), "%s/%s", run_root, workspace_id) <= 0 ||
    ...
    if (err && err_cap > 0) snprintf(err, err_cap, "%s", "workspace_binding_path_format_failed");
    return -1;
  }
  ...
  if (mkdir_if_needed(ws_root) != 0 ||
    ...
    if (err && err_cap > 0) snprintf(err, err_cap, "%s", "workspace_binding_mkdir_failed");
    return -1;
  }
