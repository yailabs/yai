...
typedef struct yai_mind_graph_store {
  yai_mind_graph_node_t *nodes;
  size_t node_count;
  size_t node_cap;
  yai_mind_graph_edge_t *edges;
  size_t edge_count;
  size_t edge_cap;
  yai_mind_node_id_t next_node_id;
  yai_mind_edge_id_t next_edge_id;
} yai_mind_graph_store_t;

static yai_mind_graph_store_t g_store = {0};

static int ensure_node_cap(size_t needed)
{
  yai_mind_graph_node_t *next;
  size_t new_cap;
  if (needed <= g_store.node_cap) return YAI_MIND_OK;
  new_cap = (g_store.node_cap == 0) ? 64 : g_store.node_cap * 2;
  while (new_cap < needed) new_cap *= 2;
  next = (yai_mind_graph_node_t *)realloc(g_store.nodes, new_cap * sizeof(*next));
...
{
  yai_mind_graph_edge_t *next;
  size_t new_cap;
  if (needed <= g_store.edge_cap) return YAI_MIND_OK;
  new_cap = (g_store.edge_cap == 0) ? 64 : g_store.edge_cap * 2;
  while (new_cap < needed) new_cap *= 2;
  next = (yai_mind_graph_edge_t *)realloc(g_store.edges, new_cap * sizeof(*next));
  if (!next) return YAI_MIND_ERR_NO_MEMORY;
  g_store.edges = next;
...
static int node_exists(yai_mind_node_id_t node_id)
{
  if (!yai_mind_node_id_is_valid(node_id)) return 0;
...
static int backend_edge_create(yai_mind_node_id_t from_node,
                               yai_mind_node_id_t to_node,
                               const char *relation,
                               float weight,
                               yai_mind_edge_id_t *edge_id_out)
{
  yai_mind_graph_edge_t *slot;
  int rc;
...
static int backend_node_get(yai_mind_node_id_t node_id, yai_mind_graph_node_t *node_out)
{
...
static int backend_edge_get(yai_mind_edge_id_t edge_id, yai_mind_graph_edge_t *edge_out)
{
  if (!edge_out || !yai_mind_edge_id_is_valid(edge_id) || (size_t)edge_id > g_store.edge_count) {
...
static int backend_query(const yai_mind_memory_query_t *query, yai_mind_memory_result_t *result_out)
{
  size_t matches = 0;
  if (!query || !result_out || !query->query[0]) return YAI_MIND_ERR_INVALID_ARG;

  for (size_t i = 0; i < g_store.node_count; i++) {
    const yai_mind_graph_node_t *n = &g_store.nodes[i];
...
static int backend_stats(yai_mind_graph_stats_t *stats_out)
{
...
static const yai_mind_graph_backend_ops_t g_ops = {
...
static const yai_mind_graph_backend_ops_t *g_selected = NULL;
