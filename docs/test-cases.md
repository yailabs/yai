...
| derived hybrid memory | `memory-representation`, engine `memory_index` tests and ignored 1k/10k/50k characterization | deterministic typed documents/profiles/artifacts, BM25, exact cosine, qualification-before-RRF, visibility/Case isolation, crash/corruption/profile replacement, eight-process publication, product drop/rebuild | semantic-quality claims, learned reranking/consolidation, admitted ANN |
