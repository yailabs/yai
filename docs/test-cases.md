...
| agentless Case runtime | `agentless-case-runtime`, engine 128-iteration residency endurance | 26 real HTTP turns, 24 controlled effects, DENY/adaptation, provider/model replacement, automatic memory repair/effect reconciliation, budget stop, bounded context and crash resume without Agent/Workflow state | real human-review pause, concurrent runs, generalized operation/carrier families |
