...
use yai_core_engine::provider_governance::{
    ProviderAttemptOutcome, ProviderRequirement, ProviderSelection,
};
...
pub(super) use yai_application::provider_execution::governed_attempt_outcome;
...
pub(super) use yai_application::provider_execution::{ProviderInvocationRefs, ControlledProviderResult};
...
fn compile_semantic_invocation(
    ...
    yai_application::provider_execution::compile_semantic_invocation(
        &yai_home(), &authenticate_local()?, &LmdbRecordStore::open(record_store_path())?,
        &session.case_ref, &session.subject_ref, &session.provider,
        purpose, task, output_contract, options,
    )
}
...
fn provider_chat_completion(
    ...
    yai_application::provider_execution::provider_chat_completion(
        &LmdbRecordStore::open(record_store_path())?, config, rendered,
        structured_json, typed_parts, output_contract, max_input_units,
    )
}

use yai_application::provider_execution::invocation_lineage;
...
fn invoke_semantic_provider_with_optional_journal(
    ...
    let (invocation, result) = yai_application::provider_execution::execute_prepared(
        &LmdbRecordStore::open(record_store_path())?, &session.provider, &semantic, options, typed_parts,
        |lineage| append_model_prompt_attempt(&session, task, lineage,
            options.workflow_execution_id.as_deref(), options.conversation_turn_id.as_deref()),
        |invocation, lineage, output| append_model_output_receipt(&session,
            &invocation.attempt_id, &invocation.invocation_id, output, lineage,
