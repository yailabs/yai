...
cleanup() {
  ...
  if [[ "${YD5_KEEP_TMP:-0}" != "1" ]]; then
    ...
    echo "daemon_local_runtime_scan_spool_retry: kept tmp dirs owner=$OWNER_HOME edge_root=$EDGE_ROOT edge_home=$EDGE_HOME"
  fi
}
trap cleanup EXIT
...
"$YAI_DAEMON" --tick-ms 120 --max-ticks 8 >/tmp/yai_yd5_edge_phase1.log 2>&1 || true

Q1="$(find "$EDGE_HOME/spool/queue" -type f 2>/dev/null || true)"
Q1="$(printf '%s\n' "$Q1" | sed '/^$/d' | wc -l | tr -d ' ')"
if [[ "$Q1" -lt 1 ]]; then
  echo "daemon_local_runtime_scan_spool_retry: debug queue_count=$Q1 edge_home=$EDGE_HOME"
  find "$EDGE_HOME" -maxdepth 4 2>/dev/null | sort | sed -n '1,220p'
  sed -n '1,220p' /tmp/yai_yd5_edge_phase1.log 2>/dev/null || true
  echo "daemon_local_runtime_scan_spool_retry: FAIL (queue not populated in disconnected phase)"
  exit 1
fi
...
[[ -S "$SOCK" ]] || { echo "daemon_local_runtime_scan_spool_retry: FAIL (owner socket not ready)"; exit 1; }
...
YAI_EDGE_SOURCE_LABEL="yd5-node-a" \
YAI_DAEMON_BINDINGS_MANIFEST="$MANIFEST" \
"$YAI_DAEMON" --tick-ms 120 --max-ticks 18 >/tmp/yai_yd5_edge_phase2.log 2>&1

D2="$(find "$EDGE_HOME/spool/delivered" -type f 2>/dev/null || true)"
D2="$(printf '%s\n' "$D2" | sed '/^$/d' | wc -l | tr -d ' ')"
if [[ "$D2" -lt 1 ]]; then
  echo "daemon_local_runtime_scan_spool_retry: FAIL (no delivered units after owner recovery)"
  exit 1
fi

if [[ ! -f "$EDGE_HOME/state/health.v1.json" ]]; then
  echo "daemon_local_runtime_scan_spool_retry: FAIL (missing daemon health state file)"
  exit 1
fi
if ! rg -n '"state":"ready"|"state":"degraded"|"state":"stopping"' "$EDGE_HOME/state/health.v1.json" >/dev/null; then
  echo "daemon_local_runtime_scan_spool_retry: FAIL (unexpected health state payload)"
  exit 1
fi

if [[ ! -f "$EDGE_HOME/state/bindings.v1.json" ]]; then
  echo "daemon_local_runtime_scan_spool_retry: FAIL (missing bindings state file)"
  exit 1
fi
if ! rg -n '"status":"active"|\"status\":\"degraded\"' "$EDGE_HOME/state/bindings.v1.json" >/dev/null; then
  echo "daemon_local_runtime_scan_spool_retry: FAIL (bindings state does not expose runtime status)"
  exit 1
fi

echo "daemon_local_runtime_scan_spool_retry: ok"
