...
YAI_BIN="$ROOT/yai"
PROVIDER_BASE_URL="${YAI_EXTERNAL_PROVIDER_BASE_URL:-${YVEX_BASE_URL:-}}"
...
printf 'run_id: %s\n' "$RUN_ID"
printf 'yai_sha: %s\n' "$YAI_SHA"
printf 'provider_mode: external_yvex\n'
printf 'proof_class: external\n'
printf 'evidence_posture: qualification\n'
printf 'exercised_shape: text_chat_completion\n'
printf 'typed_media_interlock_qualification: not_exercised\n'
...
if [[ -z "$PROVIDER_BASE_URL" || -z "$PROVIDER_MODEL" ]]; then
  printf 'external_request_attempted: false\n'
  printf 'provider_invocation_attempted: false\n'
  printf 'yvex_external_qualification_state: blocked_external_dependency\n'
  printf 'finding_class: DEPLOYMENT_LIMITATION\n'
  printf 'reason: operator endpoint and exact exposed model are required; no default target or fixture fallback\n'
  exit 3
fi

# Reject credential-bearing URLs before printing the operator-supplied target.
python3 - "$PROVIDER_BASE_URL" <<'PY'
import sys
from urllib.parse import urlsplit
u = urlsplit(sys.argv[1])
if u.scheme not in ('http', 'https') or not u.hostname or u.username or u.password or u.query or u.fragment:
    sys.exit('external endpoint must be an HTTP(S) base without credentials/query/fragment')
PY
PROVIDER_BASE_URL="${PROVIDER_BASE_URL%/}"
[[ "$PROVIDER_BASE_URL" == */v1 ]] || PROVIDER_BASE_URL="$PROVIDER_BASE_URL/v1"
printf 'provider_endpoint: %s\n' "$PROVIDER_BASE_URL"
printf 'provider_model: %s\n' "$PROVIDER_MODEL"
printf 'yvex_ref: %s\n' "${YAI_EXTERNAL_PROVIDER_REF:-unavailable}"
printf 'yvex_ref_evidence: operator_supplied_if_present_not_independently_verified\n'

timeout_seconds=$(( (PROVIDER_TIMEOUT_MS + 999) / 1000 ))
models_url="${PROVIDER_BASE_URL%/}/models"
printf 'external_request_attempted: true\n'
printf 'provider_invocation_attempted: false\n'
headers=()
if [[ -n "${YAI_EXTERNAL_PROVIDER_API_KEY:-}" ]]; then
  headers=(-H "Authorization: Bearer $YAI_EXTERNAL_PROVIDER_API_KEY")
fi
if ! models=$(curl -fsS --connect-timeout 2 --max-time 10 "${headers[@]}" "$models_url" 2>&1); then
  printf 'yvex_external_qualification_state: blocked_external_dependency\n'
  printf 'finding_class: DEPLOYMENT_LIMITATION\n'
...
started=$(date +%s%3N)
printf 'provider_invocation_attempted: true\n'
set +e
output=$(timeout "${timeout_seconds}s" env \
  YAI_JOURNAL="$journal" \
  YAI_PROVIDER_API_KEY="${YAI_EXTERNAL_PROVIDER_API_KEY:-}" \
  "$YAI_BIN" prompt --once \
...
printf 'external_provider_result_confirmed: true\n'
printf 'finding_class: NO_ISSUE\n'
