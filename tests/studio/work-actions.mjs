...
try {
 ...
 const open=async name=>{await page.getByRole('button',{name,exact:true}).click();return page.getByRole('dialog',{name});};
 ...
 const patched=await accepted('case.summary',{case_ref:caseRef});assert.ok(patched.work.nodes.some(n=>n.node_id==='additional-a'));
 ...
 await switchCase('Handoff Target');form=await open('Accept Handoff');await form.getByLabel('Handoff reference').fill(offer.handoff_id);await form.getByLabel('Source Case reference').fill(caseRef);await form.getByRole('button',{name:'Accept',exact:true}).click();await form.waitFor({state:'hidden'});
 ...
 await switchCase('Handoff Target');form=await open('Decline Handoff');await form.getByLabel('Handoff reference').fill(declinedOffer.handoff_id);await form.getByLabel('Source Case reference').fill('case:handoff-decline-source');await form.getByLabel('Reason',{exact:true}).fill('Not selected for this qualification');await form.getByRole('button',{name:'Decline',exact:true}).click();await form.waitFor({state:'hidden'});assert.equal(exchanges.findLast(item=>item.request.operation_ref==='handoff.decline').result.result_state,'success');
 ...
 console.log(JSON.stringify({result:'PASS',case_ref:caseRef,definition:definition.workflow_definition_id,patch:patchA,handoff:offer.handoff_id,proof:['Define/bind two human checkpoints','Input progression','Patch proposal and explicit adoption','Stale topology refused without mutation','Offer acknowledgement loss never redispatches','Accept/result/reconcile cross-Case','Independent decline','4 viewport matrix','CLI replay all Cases']}));
