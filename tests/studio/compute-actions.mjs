...
try {
 ...
 assert.match(await page.locator('.provider-status').textContent(),/controlled-provider/,'Unbound deployment observation must not replace the Case-bound provider');
 ...
 await page.locator('.provider-status').filter({hasText:'Endpoint responds'}).waitFor();
 const boundCatalogBefore=await page.evaluate(({tenant,target})=>window.qualificationPlatform.application.snapshot().providerCatalogs?.[JSON.stringify([tenant,target])],{tenant:'tenant:studio-ui',target:otherTarget.id});
 heldModelResponse();
 await page.waitForTimeout(50);
 const boundCatalogAfter=await page.evaluate(({tenant,target})=>window.qualificationPlatform.application.snapshot().providerCatalogs?.[JSON.stringify([tenant,target])],{tenant:'tenant:studio-ui',target:otherTarget.id});
 assert.deepEqual(boundCatalogAfter,boundCatalogBefore,'Late unbound catalog cannot replace the Case-bound target observation');
 ...
 assert.equal(await page.locator('.provider-status').getAttribute('data-state'),'reachable','Green identifies a recent endpoint response, not inference health');
 await page.getByRole('region',{name:'Deployment at a glance'}).getByText('Not observable').waitFor();
 ...
 await page.locator('.provider-status').filter({hasText:'Catalog old'}).waitFor();
 const oldCatalogBadge=page.locator('.deployment-model-observation').getByText('Catalog old',{exact:true});
 await oldCatalogBadge.waitFor();
 assert.match(await oldCatalogBadge.getAttribute('class'),/tone-warning/,'Old catalog data cannot keep an informational current badge');
 ...
 assert.equal(await page.locator('.provider-status').getAttribute('data-state'),'reachable','An empty catalog still proves an endpoint response, while the model is absent');
 catalogModels=[{id:'controlled-text-model'}];
 await page.getByRole('button',{name:'Check exposed model',exact:true}).click();
 await page.locator('.provider-status').filter({hasText:'Endpoint responds'}).waitFor();
 holdModelResponse=true;heldModelResponse=undefined;
 ...
 while(!heldModelResponse && Date.now()<refreshDeadline) await new Promise(resolve=>setTimeout(resolve,20));
 assert.ok(heldModelResponse,'Host contract refresh starts a fresh metadata check');
 assert.equal(await page.locator('.provider-status').getAttribute('data-state'),'unknown','Old catalog cannot claim reachability while the new check is unresolved');
 heldModelResponse();heldModelResponse=undefined;
 await page.locator('.provider-status').filter({hasText:'Endpoint responds'}).waitFor();
 await page.getByLabel('Selected deployment').selectOption(inventory.targets.find(item=>item.provider_key==='unbound-inventory-target').id);
 ...
 await check.getByRole('button',{name:'Refresh result',exact:true}).click();
 ...
 await history.getByText(/Completed · (evidence recorded|no capability proven|exact model not proven)/).first().waitFor();
 ...
 assert.equal(capacityRead.result.data.capacity.execution_or_resources_qualified,false);
 assert.match(await page.locator('.deployment-model-observation').innerText(),/does not reserve capacity/);
 for(const [width,height] of [[1600,960],[1440,900],[1280,800],[1000,650]]) {
   await page.setViewportSize({width,height});
   await page.screenshot({path:`${evidence}/yvex-capacity-${width}x${height}.png`});
   assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
 }
 await page.setViewportSize({width:1440,height:900});
 catalogModels[0].yvex_capacity.input_accounting='unknown';
 await page.getByRole('button',{name:'Check exposed model',exact:true}).click();
 await page.locator('.deployment-model-observation').getByRole('alert').waitFor();
 assert.equal(await page.locator('.deployment-capacity').count(),0,'Invalid optional capacity must clear the prior observation');
