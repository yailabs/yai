...
with tempfile.TemporaryDirectory(prefix='yai studio launcher ') as tmp:
    ...
    for name in ('YAI_HOME', 'GDK_BACKEND', 'WEBKIT_DISABLE_DMABUF_RENDERER', 'YAI_PROVIDER_RESPONSE_TIMEOUT_SECS'):
        env.pop(name, None)
    refused = subprocess.run(['sh', str(scripts / 'install-studio.sh')], env=env, capture_output=True)
    assert refused.returncode == 127
    def build(version):
        binary.write_text('#!/usr/bin/env python3\nimport os,json,sys\nprint(json.dumps([%r,sys.argv[1:],os.environ.get("YAI_HOME"),os.environ.get("GDK_BACKEND"),os.environ.get("WEBKIT_DISABLE_DMABUF_RENDERER"),os.environ.get("YAI_PROVIDER_RESPONSE_TIMEOUT_SECS")]))\n' % version)
        binary.chmod(0o755)
    build('first')
    ...
    assert run() == ['first', ['a b', '$(literal)'], str(Path(tmp)/'.yai'), 'x11', '1', '3600']
    build('rebuilt')
    assert run()[0] == 'rebuilt'
    assert run(dict(YAI_HOME='/explicit profile', GDK_BACKEND='wayland', WEBKIT_DISABLE_DMABUF_RENDERER='0', YAI_PROVIDER_RESPONSE_TIMEOUT_SECS='900'))[2:] == ['/explicit profile','wayland','0','900']
