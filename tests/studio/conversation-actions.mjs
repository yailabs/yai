...
try {
 ...
 const gatedNarrative=page.getByRole('region',{name:'Model explanation'});
 await gatedNarrative.getByText(/Model context needs admission before YAI can share Case evidence with the model/).waitFor();
 assert.equal(await gatedNarrative.getByRole('button',{name:'Generate explanation',exact:true}).isDisabled(),true);
 assert.equal(exchanges.filter(x=>x.request.operation_ref==='conversation.send').length,0,'Overview must not send before explicit admission');
 await gatedNarrative.getByRole('button',{name:'Open Working State',exact:true}).click();
 ...
 await page.locator('.live-rail button[aria-label="Overview"]').click();
 assert.equal(await gatedNarrative.getByRole('button',{name:'Generate explanation',exact:true}).isEnabled(),true);
 await page.getByRole('button',{name:'Conversation',exact:true}).click();
