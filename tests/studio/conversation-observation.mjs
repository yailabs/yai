...
try {
  ...
  await page.evaluate(async () => {
    ...
    const application = {
      ...
      observeConversation: async input => {
        ...
        return { result_state: 'success', data: {
          ...
          primary_result: completed ? { result_id: `result:${ref}`, output: `Retained answer for ${ref}`, invocation_id: 'invocation:fixture' } : null,
        } };
      },
    };
    const workspace = {
    ...
    const render = () => root.render(React.createElement(React.Fragment, null,
      React.createElement(ConversationView, { workspace, platform, actions: { openPerspective() {} } }),
  ...
  await page.getByText('Retained answer for request:active', { exact: true }).waitFor();
  ...
  await page.getByText('Retained answer for request:unknown', { exact: true }).waitFor();
