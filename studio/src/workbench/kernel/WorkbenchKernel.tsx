...
export function WorkbenchKernel({ workspace, stream, platform, registry, readMaterial, searchCase, refresh, openCaseSwitcher }: WorkbenchKernelProps) {
  ...
  // Keep a panel mounted after first use so local drafts and terminal sessions
  // survive tab changes, but do not initialize every hidden tool on Case open.
  const [visitedPanels, setVisitedPanels] = useState<ReadonlySet<string>>(() => new Set(["Terminal"]));
  const activatePanel = useCallback((id: string) => {
    setVisitedPanels(current => current.has(id) ? current : new Set([...current, id]));
    setPanel(id);
  }, []);
  ...
  const [bottomHeight, setBottomHeight] = useState(() => Math.max(190, Math.floor(window.innerHeight * (platform.configuration.get<number>("workbench.panel.heightRatio") ?? .24))));
  ...
  useEffect(() => {
    const guard = (event: Event) => {
      if (windowSession.dirtyCount && !window.confirm(`Leave Studio and discard ${windowSession.dirtyCount} unsaved file buffer(s)?`)) event.preventDefault();
    };
  ...
  useEffect(() => {
    ...
    const terminal = (name: "new" | "kill" | "clear" | "focus") => { setSurfaceFocused(false); activatePanel("Terminal"); setBottomOpen(true); requestAnimationFrame(() => dispatchTerminalCommand(name)); };
  ...
  }, [activatePanel, applyLocation, closeSurface, containers, contextTools, dispatchSurfaceCommand, openCaseSwitcher, openPerspective, openSettings, platform, refresh, surfaceState.activeId, surfaceState.inputs, surfaces, toggleRegion, toggleSurfaceFocus]);
  ...
  useEffect(() => platform.configuration.subscribe(key => {
    ...
    if (key === "workbench.panel.heightRatio") {
      const height = Math.max(190, Math.floor(window.innerHeight * (platform.configuration.get<number>(key) ?? .24)));
  ...
  // Renderer-local results/dialogs belong to one disclosure context. File drafts
  // remain in the separately scoped WorkbenchSession, not in this React lifetime.
  const surfaceContextKey = JSON.stringify([workspace.case.case_ref, workspace.case.participant_ref]);
  ...
  const hostExecutableReplaced = hostState.state === "live" && hostState.telemetry?.executable_posture === "replaced_on_disk";
  return <div className="live-case-shell workbench-kernel" data-case-ref={workspace.case.case_ref} data-surface-focused={surfaceFocused} data-case-source={workspace.presentation.dataKind} data-host={platform.host.capabilities.kind} style={{ "--left-width": `${leftWidth}px`, "--right-width": `${rightWidth}px`, "--bottom-height": `${bottomHeight}px` } as React.CSSProperties}>
    ...
    <div className="live-workbench">
      ...
      <section className="live-center"><SurfaceTabs inputs={surfaceState.inputs} activeId={surfaceState.activeId} describe={(input) => { const renderer = registry.surfaceRenderer(input.surfaceType); return {archetype: renderer?.archetype, role: renderer?.role ?? "content", pinnable: renderer?.capabilities.includes("pinnable") ?? false}; }} activate={openSurface} pin={(id) => surfaces.pin(id)} close={closeSurface} /><main className="live-surface" data-archetype={surfaceRenderer?.archetype ?? "product"} tabIndex={-1}>{activeInput && SurfaceComponent ? <Suspense fallback={<div className="surface-loading">Loading {activeInput.title}…</div>}><SurfaceComponent key={surfaceContextKey} {...renderContext} input={activeInput} /></Suspense> : <div className="empty-surface"><h2>{activeInput ? "Surface unavailable" : "Your workspace is ready"}</h2><p>{activeInput ? `No trusted renderer is registered for ${activeInput.title}.` : "Open a Case perspective from the sidebar or find a file with Quick Open."}</p>{!activeInput && <button onClick={() => setSearchMode("open")}>Quick Open</button>}</div>}</main>
        ...
        <section hidden={!panelVisible} className="live-bottom" id="case-tools" style={{ height: bottomHeight }} aria-label="Bottom tools"><header><div className="panel-tabs" ref={panelTabs}>{panelViews.map((view) => <button key={view.id} aria-pressed={panel === view.id} onClick={() => activatePanel(view.id)}>{view.title}</button>)}</div><div className="panel-contribution-toolbar" ref={setPanelToolbarTarget} /><IconButton aria-label={bottomMaximized ? "Restore bottom panel" : "Maximize bottom panel"} onClick={() => { if (bottomMaximized) { setBottomHeight(restoredBottomHeight.current); setBottomMaximized(false); } else { restoredBottomHeight.current = bottomHeight; setBottomHeight(bottomLimit); setBottomMaximized(true); } }}><Icon name={bottomMaximized ? "restore" : "maximize"} size={14} /></IconButton><IconButton aria-label="Close bottom panel" onClick={() => setBottomOpen(false)}><Icon name="close" size={14} /></IconButton></header><div className="tool-content">{panelViews.map((view) => { const Component = view.component; const visible = panelVisible && activePanel?.id === view.id; return <div key={view.id} className="tool-pane" hidden={!visible}>{visitedPanels.has(view.id) && <Component {...renderContext} visible={visible} available={platform.host.capabilities.terminalAvailable} toolbarTarget={visible ? panelToolbarTarget : null} closePanel={() => setBottomOpen(false)} />}</div>; })}</div></section>
      </section>
