...
export function SurfaceTabs({ inputs, activeId, describe, activate, pin, close }: {
  ...
  activeId?: string;
  describe(input: SurfaceInput): { role: SurfaceRole; pinnable: boolean };
  ...
  return <div ref={strip} className="surface-tabs" role="tablist" aria-label="Open work surfaces" onKeyDown={(event) => {
    ...
    return <div key={input.id} className={`surface-tab surface-role-${descriptor.role} surface-posture-${input.posture ?? "ready"}`} data-active={activeId === input.id}>
