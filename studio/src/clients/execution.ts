...
export function readExecutionRefs(key: string): ExecutionReference[] {
  try { const items: unknown = JSON.parse(sessionStorage.getItem(key) ?? "[]"); return Array.isArray(items) ? items.filter((item): item is ExecutionReference => item && (item.domain === "source_acquisition" ? typeof item.source_ref === "string" && Number.isSafeInteger(item.attempt) && item.attempt > 0 : item.domain === "cognitive_realization" ? typeof item.plan_ref === "string" && item.plan_ref.length > 0 : ["runtime_work", "resource_request"].includes(item.domain) && typeof item.submission_ref === "string")).slice(-12) : []; } catch { return []; }
}
