...
export function CollectionList({ label, items, selected, onSelect, empty = "No items in this projection", pageSize = 40 }: {
  ...
  return <section className="collection-list" aria-label={label}>
    ...
    <div className="collection-items">{visible.map(item => <button type="button" className={`collection-row ${selected === item.id ? "selected" : ""}`} key={item.id} onClick={() => onSelect(item.id)} aria-pressed={selected === item.id}>
