...
import { requestStudioRestart } from "../platform/windowClose";
...
export function StudioApplication({ dataSource, platform, registry }: { dataSource: CaseDataSource; platform: PlatformServices; registry: WorkbenchRegistry }) {
  ...
  useEffect(() => {
    ...
    if (platform.host.capabilities.nativeDesktop) {
      registered.add(platform.commands.registerCommand({ id: "studio.app.restart", title: "Restart Studio", handler: requestStudioRestart }));
      registered.add(platform.menus.registerMenuItem({ id: "application:studio.app.restart", location: "File", group: "9", command: "studio.app.restart", order: 0 }));
    }
  ...
  return <div className={`live-studio ${attachment ? "case-attached" : ""}`}>
    ...
    {!attachment && <StartCenter result={openFailure ?? catalog} cases={cases} dataKind={dataSource.kind} open={loadCase} retry={() => openFailure && requestedCase.current ? void loadCase(requestedCase.current) : void loadCases()} refresh={() => void loadCases()} newCase={dataSource.composition ? () => setComposer(true) : platform.application?.supports("case.create") ? () => setProductAction("create") : undefined} />}
...
function StartCenter({ result, cases, dataKind, open, retry, refresh, newCase }: { result?: OperationResult<unknown>; cases: LiveCaseRow[]; dataKind: "live" | "fixture"; open: (id: string) => void; retry: () => void; refresh: () => void; newCase?: () => void }) {
  const [query, setQuery] = useState("");
  const [showClosed, setShowClosed] = useState(false);
  const closedCount = cases.filter(item => item.case_status === "closed").length;
  const visible = cases.filter(item => (showClosed || item.case_status !== "closed") && `${item.case_ref} ${item.display_name}`.toLowerCase().includes(query.toLowerCase()));
  return <main className="live-start"><section className="live-start-intro"><span>{dataKind === "live" ? "Local Case Workbench" : "Fixture development Workbench"}</span><h1>Open a Case.</h1><p>{dataKind === "live" ? "Studio attaches to durable Case continuity owned by YAI on this machine." : "Explicit authored Case data runs through the same Workbench as live data."}</p><SearchInput autoFocus aria-label="Search Cases" placeholder={dataKind === "live" ? "Search real local Cases" : "Search fixture Cases"} value={query} onChange={(event) => setQuery(event.target.value)} />{newCase && <Button onClick={newCase}>{dataKind === "live" ? "New Case" : "Compose fixture Case"}</Button>}</section>{result?.result_state && result.result_state !== "success" ? <HostFailure result={result} retry={retry} /> : <section className="live-case-list" aria-label={dataKind === "live" ? "Real local Cases" : "Fixture Cases"}><PanelHeader title={dataKind === "live" ? "Local Cases" : "Fixture Cases"} detail={`${visible.length} available`} actions={<span className="case-catalog-actions">{closedCount > 0 && <Button onClick={() => setShowClosed(value => !value)}>{showClosed ? "Hide closed" : `Show ${closedCount} closed`}</Button>}<IconButton aria-label="Refresh Case list" onClick={refresh}><Icon name="refresh" /></IconButton></span>} />{visible.map((item) => <button className="live-case-row" data-case-ref={item.case_ref} key={item.case_ref} onClick={() => open(item.case_ref)}><Icon name="case" size={18} /><span><strong>{item.display_name}</strong><small>{item.case_status}</small></span><span className="case-facts">{item.source_count} sources · {item.participant_count} participants</span><Icon name="chevron" size={14} /></button>)}{!result ? <p role="status" className="surface-loading">Loading Cases…</p> : !visible.length && <EmptyState title={cases.length ? "No matching Cases" : "No Cases"} body={cases.length ? "Try another name or show closed Cases." : dataKind === "live" ? "YAI returned an empty authorized Case list. Studio has not substituted sample data." : "No authored Cases are available."} />}</section>}<footer><Badge tone={dataKind === "live" ? "success" : "warning"}>{dataKind === "live" ? "Real local data" : "Fixture Case data"}</Badge><span>{dataKind === "live" ? "No fixture fallback." : "Explicit development mode."}</span></footer></main>;
}
...
function CaseSwitcher({ cases, dataKind, close, open }: { cases: LiveCaseRow[]; dataKind: "live" | "fixture"; close: () => void; open: (id: string) => void }) { const root = useModalFocus(close); const [query, setQuery] = useState(""); const [showClosed, setShowClosed] = useState(false); const closedCount = cases.filter(item => item.case_status === "closed").length; return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section ref={root} className="case-switcher" role="dialog" aria-modal="true" aria-label="Open Case"><PanelHeader title="Open Case" detail={dataKind === "live" ? "Real local YAI" : "Explicit fixture data"} actions={<IconButton aria-label="Close" onClick={close}><Icon name="close" /></IconButton>} /><SearchInput autoFocus placeholder="Search Cases" value={query} onChange={(event) => setQuery(event.target.value)} />{closedCount > 0 && <Button onClick={() => setShowClosed(value => !value)}>{showClosed ? "Hide closed" : `Show ${closedCount} closed`}</Button>}{cases.filter((item) => (showClosed || item.case_status !== "closed") && `${item.display_name} ${item.case_ref}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())).map((item) => <button className="ui-list-row" key={item.case_ref} data-case-ref={item.case_ref} title={item.case_ref} onClick={() => open(item.case_ref)}><Icon name="case" /><span><strong>{item.display_name}</strong><small>{item.case_status}</small></span></button>)}</section></div>; }
