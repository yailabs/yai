...
const sections: readonly SettingSection[] = ["General", "Appearance", "Workbench", "Editor", "Terminal", "Identity", "YAI Host", "Security", "Advanced"];
...
export function SettingsSurface({ workspace, actions, platform, settings, selection }: SurfaceRendererProps) {
  ...
  return <div className="settings-surface" data-surface-type="studio.settings">
    ...
    <div className="settings-content"><span>{query ? "Search results" : "Studio settings"}</span><h2>{query ? `Results for “${query}”` : section}</h2><p className="settings-scope-note">Local preferences stay in Studio. Case and host settings require their qualified owners.</p>{!query && section === "YAI Host" && <YaiHostSettings platform={platform} />}{!query && section === "Advanced" && <ApplicationCapabilities platform={platform} />}{!query && section === "Identity" && <IdentitySettings workspace={workspace} platform={platform} />}{!query && section === "General" && <section className="setting-row"><div><h3>Provider actions</h3><p>{application.catalog?.operations.some(item => item.operation_id.startsWith("provider.")) ? "Provider and runtime operations belong in Compute, alongside models and Case target bindings." : "The connected Host does not currently advertise provider governance operations."}</p><button className="ui-button" onClick={() => actions.openPerspective("Compute")}>Open Compute</button></div></section>}{(query || section !== "YAI Host") && visible.map((definition) => <SettingRow key={definition.id} definition={definition} value={platform.configuration.get(definition.id) ?? definition.defaultValue} update={(value) => platform.configuration.update(definition.id, value)} dataKind={workspace.presentation.dataKind} backend={workspace.presentation.backendPosture} />)}{(query || section !== "YAI Host" && section !== "Identity") && !visible.length && <EmptyState title="No matching settings" body="Try a setting title, description or section." />}</div>
  </div>;
}
