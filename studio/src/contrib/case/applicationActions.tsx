...
export function WorkflowInputAction({ application, workspace, nodeRef, refresh }: { application?: ApplicationAccess; workspace: CasePresentation; nodeRef: string; refresh(): void | Promise<void> }) {
  ...
  const definition = workspace.work.effective_nodes ? workspace.work.effective_nodes.find(node => node.node_id === nodeRef)?.node : workspace.work.definition?.nodes?.find((node): node is { node_id: string; kind: string; prompt?: string; max_bytes?: number } => Boolean(node && typeof node === "object" && "node_id" in node && node.node_id === nodeRef && "kind" in node && node.kind === "human_input"));
  if (!application) return null;
