...
export function WorkSurface({ workspace, platform, actions }: Pick<SurfaceRendererProps, "workspace" | "platform" | "actions">) {
  ...
  return <article className="live-page work-surface"><header className="operational-heading"><div><h1>Work</h1><p>Structured progression, Case handoffs and committed operational activity.</p></div><Badge>{nodes.length} workflow nodes</Badge></header>
    <RunCaseAction workspace={workspace} platform={platform} /><ExecutionHistory workspace={workspace} platform={platform} /><section className="work-section"><h2>Workflow</h2><WorkflowActions workspace={workspace} platform={platform} />
    ...
    </section><DecisionHistory key={`${workspace.case.case_ref}:${workspace.case.participant_ref}:${workspace.case.generation}`} workspace={workspace} platform={platform} actions={actions} /><HandoffActions workspace={workspace} platform={platform} />
    <section className="work-section"><h2>Recent operational history</h2><p>Latest {events.length} matching events from the bounded Case history projection. An event is not a live execution handle.</p>{events.map(event => <button className="fact-row" key={event.id} onClick={() => actions.inspect(event.id)}><Icon name="memory" /><span><strong>{event.kind.replaceAll("_", " ")}</strong><small>{event.component}</small></span></button>)}</section>
    <section className="work-section"><h2>Executions & effects</h2><p>Runtime work, Source attempts and governed Resource requests retain distinct domain identities. Observe exact submissions above; Source acquisition and Resource requests are available from their Environment objects. Conversation sends through the Case’s governed model assignment and retains an execution identity. A lost response never triggers automatic redispatch.</p></section>
  </article>;
}
