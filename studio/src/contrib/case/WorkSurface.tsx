...
export function WorkSurface({ workspace, platform, actions }: Pick<SurfaceRendererProps, "workspace" | "platform" | "actions">) {
  ...
  return <article className="live-page work-surface"><header className="operational-heading"><div><h1>Work</h1><p>Structured progression, Case handoffs and committed operational activity.</p></div><Badge>{nodes.length} workflow nodes</Badge></header>
    ...
    <section className="work-section"><h2>Executions & effects</h2><p>Runtime work, Source attempts and governed Resource requests retain distinct domain identities. Observe exact submissions above; Source acquisition and Resource requests are available from their Environment objects. Conversation SEND and direct provider execution are still separate unpublished boundaries.</p></section>
  </article>;
}
