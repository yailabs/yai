...
function Narrative({ workspace, platform, configure, inspect }: { workspace: CasePresentation; platform: PlatformServices; configure(): void; inspect(ref: string): void }) {
  ...
  useEffect(() => {
    setResult(undefined);
    if (!request || !app?.supports("execution.get")) return;
    let disposed = false; let timer: ReturnType<typeof setTimeout>;
    const observe = async () => {
      const response = await app.observeConversation({case_ref, participant_ref, execution:{domain:"conversation", submission_ref:request.submission_ref}});
      if (disposed) return;
  ...
  }, [app, availability, case_ref, participant_ref, request, generation, tick]);
    ...
    try {
      ...
      if (response.result_state !== "success") {
        ...
        if (["unauthorized", "stale", "not_implemented"].includes(response.result_state)) {
          sessionStorage.removeItem(key); setRequest(undefined);
        }
      }
  ...
  const unfinished = request && (!result || ["admitted", "running", "unresolved", "delivery_indeterminate"].includes(result.posture));
