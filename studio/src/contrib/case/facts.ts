...
export function findFact(workspace: CasePresentation, id: string) {
  ...
  const event = workspace.memory.timeline.find((item) => item.id === id); if (event) return { title: humanize(event.kind), detail: event.summary ?? "Committed Case transition", values: values(["Generation", String(event.sequence)], ["Owner", event.component]) };
  ...
  const provider = workspace.compute.targets.find((item) => item.id === id); if (provider) return { title: provider.provider_key, detail: provider.model_id, values: values(["Adapter", provider.adapter], ["Locality", provider.locality], ["Endpoint", provider.endpoint]) };
...
export function factKind(workspace: CasePresentation, id: string) { if (workspace.knowledge.units.some(item => item.id === id)) return "knowledge unit"; if (workspace.knowledge.sources.some(item => item.id === id)) return "knowledge document"; if (workspace.knowledge.entities.some(item => item.id === id)) return "entity"; if (workspace.knowledge.contradictions.some(item => item.id === id)) return "contradiction"; if (id === workspace.case.case_ref) return "case"; if (workspace.presentation.materials?.some((item) => item.id === id)) return "material"; if (workspace.environment.files.some((item) => item.id === id)) return "file"; if (workspace.environment.sources.some((item) => item.id === id)) return "source"; if (workspace.environment.resources.some((item) => item.id === id)) return "resource"; if (workspace.overview.participants.some((item) => item.id === id)) return "participant"; if (workspace.authority.policies.some((item) => item.id === id)) return "policy"; if (workspace.authority.reviews.some((item) => item.id === id)) return "review"; if (workspace.compute.targets.some((item) => item.id === id)) return "provider"; if (workspace.work.nodes.some((item) => item.node_id === id)) return "workflow"; if (workspace.memory.timeline.some((item) => item.id === id)) return "event"; return "case fact"; }
export function factIcon(kind: string): IconName { return ({ "knowledge unit": "knowledge", "knowledge document": "file", entity: "knowledge", contradiction: "review", case: "case", material: "file", file: "file", source: "source", resource: "resource", participant: "people", policy: "authority", review: "review", provider: "compute", workflow: "work", event: "memory" } as Record<string, IconName>)[kind] ?? "overview"; }
