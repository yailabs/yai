...
export function findFact(workspace: CasePresentation, id: string) {
  ...
  if (id === workspace.case.case_ref) return { title: workspace.case.display_name, detail: "Durable continuity and application truth owned by YAI.", values: values(["Case", workspace.case.case_ref], ["Lifecycle", workspace.case.case_status], ["State version", String(workspace.case.generation)], ["Attached participant", workspace.case.participant_ref], ["Data source", workspace.presentation.dataKind]) };
  ...
  const event = workspace.memory.timeline.find((item) => item.id === id); if (event) return { title: humanize(event.kind), detail: event.summary ?? "Committed Case transition", values: values(["Transition", event.id], ["State version", String(event.sequence)], ["Committed", new Date(event.committed_at_unix_ms).toISOString()], ["Owner", event.component], ["Participant", event.participant_ref ?? "Not supplied"]) };
