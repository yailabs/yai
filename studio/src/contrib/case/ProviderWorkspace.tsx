        ...
        {item ? <><header className="deployment-heading"><div><small>Selected deployment</small><h2>{item.provider_key}</h2></div><Button onClick={() => actions.inspect(item.id)}>Inspect deployment</Button></header>
          ...
          {section === "Runtime" && <>
            <section className="deployment-health" aria-label="Observed deployment health"><header><h3>Observed health</h3><Badge tone={tone}>{health?.posture ?? "Not observed"}</Badge></header>
              ...
              <small>Recorded observation, not a live connectivity probe.</small>
            </section>
          ...
          {section === "Evidence" && <><section><h3>Qualified capabilities</h3>{posture?.qualification ? <><ul className="deployment-capabilities">{posture.qualification.capabilities.map(capability => <li key={capability.capability}><strong>{capability.capability}</strong><span>{capability.provenance}</span></li>)}</ul><p>Measured {time(posture.qualification.qualified_at_unix_ms)}</p><code>{posture.qualification.run_id}</code></> : <p>No qualification is exposed for this target.</p>}</section><section><h3>Governance</h3><p>These actions apply to the selected deployment. Trust does not grant execution authority.</p>{controls(item.id)}</section><Button onClick={() => actions.inspect(item.id)}>Inspect exact identities</Button></>}
