    ...
    <div className="provider-workspace-body">
      ...
      <section className="deployment-workspace" aria-label="Deployment details">
        {item ? <><header className="deployment-heading"><div><small>Selected deployment</small><h2>{item.provider_key}</h2><Badge tone={posture?.trust?.posture === "approved" ? "success" : posture?.trust?.posture === "denied" ? "error" : "neutral"}>{posture?.trust?.posture ?? "Unreviewed"}</Badge></div><Button onClick={() => actions.inspect(item.id)}>Inspect deployment</Button></header>
          ...
          {(conversationUnqualified || circuitOpen) && <div className="deployment-route-alert" role="status"><strong>Case chat is blocked for this deployment.</strong><span>{conversationUnqualified ? "The last small text-interface check is missing or failed." : ""}{circuitOpen ? " YAI paused requests after repeated failures." : ""} Seeing the model in the catalog does not prove it can answer a Case request.</span><Button onClick={() => setSection("Evidence")}>See checks and failures</Button></div>}
