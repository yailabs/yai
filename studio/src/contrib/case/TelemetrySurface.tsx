...
export function TelemetrySurface({ workspace, platform, actions }: SurfaceRendererProps) {
  ...
  return <article className="telemetry-surface">
    ...
    <section id="telemetry-host" {...panel("Host")}><header><h2>YAI Host</h2><Badge tone={host?.executable_posture === "replaced_on_disk" ? "warning" : host ? "success" : "warning"}>{host?.executable_posture === "replaced_on_disk" ? "Host binary changed" : connection.state}</Badge></header>
