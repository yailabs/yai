...
export function PerspectiveSurface({ workspace, input, actions, selection }: SurfaceRendererProps) {
...
function Work({ workspace, inspect }: { workspace: CasePresentation; inspect: (id: string) => void }) { const nodes = workspace.work.nodes.map((node) => ({ id: node.node_id, label: compact(node.node_id), kind: node.node_kind })); return <div className="live-page"><SurfaceHeader workspace={workspace} title="Work" body="Workflow definition and current progression remain distinct." />{workspace.work.status === "empty" ? <EmptyState title="No workflow is configured" body={workspace.work.message ?? "No workflow binding is exposed."} /> : <><GraphViewport key={workspace.case.case_ref} nodes={nodes} edges={workspace.work.edges} layout="directed" onInspect={inspect} /><FactSection title="Current work" empty="No work nodes">{workspace.work.nodes.map((node) => <button className="fact-row" key={node.node_id} onClick={() => inspect(node.node_id)}><Icon name="work" /><span><strong>{node.node_id}</strong><small>{node.reason}</small></span><Badge>{node.posture}</Badge></button>)}</FactSection></>}</div>; }
