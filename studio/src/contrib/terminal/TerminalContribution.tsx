...
export function TerminalPanelView({ available, platform, toolbarTarget, closePanel }: PanelViewProps) {
  return available ? <TerminalPanel scrollback={platform.configuration.get<number>("terminal.scrollback") ?? 5000} toolbarTarget={toolbarTarget} onEmpty={closePanel} /> : <EmptyState title="Terminal requires desktop host" body="The browser Workbench does not create or emulate a shell." />;
}
