...
import { arrangeGraph, completeGraph, graphSlice, graphBounds, fitGraphCamera, type GraphLayout } from "../contrib/case/graph";
...
export function GraphViewport({ nodes, edges, layout, onInspect }: {
  ...
  const viewport = useMemo(() => graphSlice(filtered, query, focus, page, pageSize), [filtered, query, focus, page, pageSize]);
  ...
  const bounds = graphBounds(initialPositions);
  ...
  return <section className="graph-viewport" aria-label={`${layout} Case graph`}>
    <div className="graph-tools">
      ...
      <Button onClick={() => setCamera(fitGraphCamera(arranged, initialPositions, size))}>Fit</Button>
    ...
    <div className="graph-filters"><select aria-label="Graph object kind" value={kind} onChange={event => { setKind(event.target.value); setFocus(undefined); setPage(0); reset(); }}><option value="">All object kinds</option>{[...new Set(graph.nodes.map(node => node.kind).filter(Boolean))].sort().map(value => <option key={value}>{value}</option>)}</select><select aria-label="Graph relation kind" value={relation} onChange={event => { setRelation(event.target.value); setFocus(undefined); setPage(0); reset(); }}><option value="">All relation kinds</option>{[...new Set(graph.edges.map(edge => edge.kind))].sort().map(value => <option key={value}>{value}</option>)}</select><select aria-label="Visible graph object budget" value={pageSize} onChange={event => { setPageSize(Number(event.target.value)); setPage(0); reset(); }}>{[24,48,96].map(value => <option key={value} value={value}>Up to {value} objects</option>)}</select><span>{graph.nodes.filter(node => node.referenceOnly).length} unresolved references</span></div>
    ...
    <svg ref={canvas} viewBox={`0 0 ${size.width} ${size.height}`} role="group" aria-label="Graph objects and directed relations"
      ...
      <g transform={`translate(${offset.x} ${offset.y}) scale(${scale})`}>
        ...
        {arranged.map((node, index) => <g key={node.id} data-object-id={node.id} className={`graph-node ${overview ? "overview" : ""} ${node.id === selected ? "selected" : ""} ${selected && node.id !== selected && !neighbors.has(node.id) ? "muted" : ""} ${node.referenceOnly ? "reference-only" : ""}`} transform={`translate(${node.x} ${node.y})`} tabIndex={0} role="button" aria-pressed={node.id === selected} aria-label={`${node.label ?? node.id}, ${node.kind ?? "node"}`} onClick={() => select(node.id)} onKeyDown={event => {
          ...
          <title>{`${node.label ?? node.id}\n${node.kind}${node.referenceOnly ? " · reference only" : ""}\n${node.id}`}</title><rect x={overview ? -14 / scale : -90} y={overview ? -14 / scale : -20} width={overview ? 28 / scale : 180} height={overview ? 28 / scale : 54} rx={overview ? 7 / scale : 7} /><circle cx={overview ? 0 : -76} cy={overview ? 0 : -2} r={overview ? 5 / scale : 4} /><text x={overview ? 12 / scale : -63} y={overview ? 4 / scale : scale < .7 ? 12 : 2} className="node-label" style={{ fontSize: overview ? 12 / scale : labelSize }}>{(node.label ?? node.id).slice(0, overview ? 36 : labelLimit)}{(node.label ?? node.id).length > (overview ? 36 : labelLimit) ? "…" : ""}</text>{scale >= .7 && <text x={-63} y={20} className="node-kind">{node.kind?.replaceAll("_", " ")}</text>}
        </g>)}
      </g>
    </svg>
