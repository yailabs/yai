...
**NOW:** continue infrastructure Case/behavioral portfolio, Tenant Providers and Case Compute
...
| SE02 | 🟡 PARTIAL | Source declare/acquire/resume/revoke and explicit policy-source publication with exact attempt recovery | Real HTTP carrier loss remains indeterminate with no redispatch; Review-approved same-attempt continuation, retained material and current revocation are qualified; automatic uncertain-delivery recovery remains open | environment-actions.mjs; source-policy-actions.mjs; execution-actions.mjs |
| SE03 | 🟡 PARTIAL | Resource kind/bounds/requestability, process attachment and seven authored request variants with receipt observation | Six authored owner-resolved `resource.import` families including exact-digest process runners with Host retry/refusal proof; reviewed process-request continuation qualified across Host restart, stale confirmation and lost acknowledgement; runner environment/write-envelope setup, content admission and catalog-qualified MCP calls remain interaction debt | source-policy-actions.mjs; effect-actions.mjs; resource-setup.mjs |
...
| SK02 | 🟡 PARTIAL | Full-canvas associative graph, 24/48/96-object budgets, semantic zoom, drag-preserving Fit, filters, neighborhoods, edges and unresolved refs | Backend projection paging and cross-page relationship exploration remain | knowledge-navigation.mjs |
...
| SO01 | 🟡 PARTIAL | Work has Case-local Workflow/Executions/Handoffs/Decisions/Activity sections, bounded run/stop/resume, exact execution observation and historical Decision reconstruction with current controlled-effect receipts | Output now observes exact retained Conversation responses with selected Turn and restart recovery; Resource process exit and opt-in stdout/stderr qualified separately from receipt outcome; Work/Executions automatically discover bounded current-authorized references after restart and select one exact observation; a complete execution catalog, outgoing Handoff catalog and general result-body browsing remain open; retained-result file/process proposal and submit qualified; filesystem reconciliation qualified; process recovery interaction remains open | work-actions.mjs; effect-actions.mjs; conversation-actions.mjs; retained-output.mjs; execution-discovery.mjs; resource-setup.mjs |
| SO02 | 🟡 PARTIAL | Define/bind human checkpoint workflows, HumanInput, checkpoint patch propose/adopt | Adopted topology edges and exact checkpoint prompts are projected from engine resolution; other node authors remain open | work-actions.mjs |
| SO03 | 🟡 PARTIAL | Explicit Handoff offer/accept/decline/result/reconcile | Incoming inbox and selected request/result details use shared CLI/Application owner reads; exact refs prefill Accept/Decline; complete outgoing/history catalog remains open | work-actions.mjs; handoff-lifecycle.mjs |
...
| SP02 | 🟡 PARTIAL | Full-height deployment workspace with Runtime/Evidence/Platform; selected-target Inspector, timed health/failure observations and bounded refresh | Tenant target inventory and exposed model discovery are typed; typed durable synthetic checks are consumed with lost-ack, exact-retry and wrong-model refusal proof; per-Case usage aggregation remains open | compute-actions.mjs |
| SP03 | 🟡 PARTIAL | Cognitive preparation distinguished from provider configuration | Compute prepares retained Turn input and explicitly realizes its exact plan; text, lost ACK, stale and hidden refusals qualified. Auxiliary setup, composition and multimedia success remain debt | Explicit Compute posture |
...
| SH02 | 🟡 PARTIAL | Host supervises the existing RuntimeInstance; exact run/stop and reconnect observation | Native auto-start shares the CLI Rust composition; S1 lifecycle depth and ambiguous-delivery recovery remain open | native-portfolio.py --desktop-autostart; context_capacity.py --desktop-host; effect-actions.mjs; application-execution-lifecycle characterization |
