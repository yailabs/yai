...
| SH01 | 🟢 ESTABLISHED | Resident same-user Unix Host, singleton discovery, events, auto-attach, telemetry and lifecycle | Linux clients now identify a replaced Host executable from the exact PID/process identity and surface Restart needed without interrupting active work; a linked executable does not prove source-version equality | application/yai-host tests; host-executable-posture.py; existing native acceptance |
