...
| Disposable portfolio / native update recovery | Two native Studios on one Host; fresh manifest-derived profile only | `native-portfolio.py --fresh-profile`: canonical CLI mutation reaches both Journals, exact retry leaves history unchanged, Host restart replaces attachments, a subsequent mutation reaches both clients, drafts survive and the parallel Case stays unchanged. No model execution or operator Case mutation. |
