...
| Next decision point | Public YAI ↔ YVEX cognitive-state boundary remains UNSELECTED. Bounded semantic refresh is qualified; no automatic successor, provider convergence or B1 execution. Compact live-model qualification stays independently NOT QUALIFIED. |
...
| C02 | Authority/disclosure-aware general compilation and refresh | 🟡 PARTIAL | Current admitted view, mandatory selection and stale-W refusal; [same-EffectivePolicy projection](https://github.com/yailabs/yai/blob/c187648e9d9909d4d9b6711f131fa726d48cd585/refoundation/validation/governance-cognitive-context/REPORT.md) adds current rules and atomic policy-only revoke refusal before invocation. [Explicit same-task refresh](docs/recall.md#prompt-independent-semantic-refresh) now requalifies W3/W4 through existing Recall and full compilation, including same-generation visibility/backing change and freshly qualified paging preferences. [CLI oracle](tests/characterization/source-grounded-knowledge/test_refresh.py) and typed policy/as-of negatives qualify bounded preflight, not a freshness lease. Automatic consumer invalidation, State Read and bounded-staleness enrichment remain targets; live model understanding is unqualified. |
...
| **C — State Compilation & Cognitive Boundary**: compile governed semantic working state | S-only W2, Recall-aware W3, opt-in exact-group W4/Page1, explicit same-task refresh, derived delta/full fallback and context-compatible lowering | Broader task sufficiency/demand expansion, automatic refresh consumers and public E contracts | Preserve Recall discovery versus exact paging and W execution selection; current control and exact evidentiary groups precede optional relevance; semantic sufficiency is not target capacity | Full/ΔW equivalence, mandatory state and current authority/backing with atomic scoped closure; later public E reconciliation | No latent writes, model adapters, physical layouts or YVEX placement |
...
| Prompt-independent working/E refresh | PARTIAL W; E OPEN | [Explicit W3/W4 refresh](docs/recall.md#prompt-independent-semantic-refresh) reuses the stored task and rebuilds current Recall/W, with current policy/disclosure, exact backing and historical-cut qualification. Existing family coverage only; SemanticDelta v1 remains S-only, and identical S does not imply unchanged R. Provider/Workflow consumer convergence, automatic invalidation and future E reconciliation remain unqualified. No qualified removal means invalidate/reconstruct. |
...
consumption. Provider/Conversation/Workflow still use their S-only W2 path.
...
| Now | Bounded prompt-independent semantic refresh | Explicit typed/CLI preflight reuses W3/W4 task semantics, current qualification and full Recall/W reconstruction. No prompt, model, new store or provider migration; optional page preferences survive only over freshly qualified exact groups |
| Near / Mid | Automatic refresh consumers / broader demand expansion | No automatic selection. General historical validity, all-owner coverage, consumer convergence and safe removal remain pressure; learned navigation is optional |
...
| Golden local product lifecycle | Current PASS, deterministic model with real local persistence/resource adapters and protocol peers; exact Invocation → W and rebuild equality | Bounded free/Workflow, authority, replay/rebuild/recovery/isolation lifecycle; separate Golden lane, not folded into `make check`; provider W2 remains independent of explicit paging |
...
| Semantic-state refoundation | Implementation/local qualification PASS; external closure BLOCKED | Replay-qualified composition, W and delta/full fallback remain earned; complete external Golden is still required. No universal locality or E_t proof |
...
| Continuous source synchronization | False. Explicit revision refresh, source revoke and bounded resume are qualified in O08; M07/Recall v2 revalidate on each query. Explicit same-task W refresh is qualified, but no watcher, synchronization daemon, background refresh or automatic consumer invalidation/refresh integration is implemented. |
