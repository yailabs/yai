...
YAI: admitted world/Case events → canonical history/current state → S_t
  │
  │ State Compilation: intent + authority/disclosure + relevance + budget
  ▼
...
| YVEX / execution substrate | Exact model/deployment/runtime truth; target realization and lifecycle of experiential computational E_t, layout, State Read/Update, KV/recurrent/SSM/latent state, paging/residency, checkpoints, rollback/invalidation, layers/kernels and physical evidence | the entire Case by implication, Case authority, semantic memory ownership, Policy, Workflow truth or a shared canonical database |
...
```text
S_t = Semantic State of the Case, under existing YAI admission rules
W_t = Compile(S_t, I_t, A_t, R_t, B_t)
E_t = Lower(W_t, Model, StateProfile)
Exec(X_t, E_t) → (Y_t, E_t+1, P_t optional)
```

I_t is current intent, A_t Participant authority/disclosure, R_t task relevance
and B_t the execution budget; X_t is immediate input and Y_t computational
output. These symbols and Model/StateProfile are conceptual notation, not source
types or fixed API parameters, and are distinct from the program letters below.

**S_t** is durable model-independent Case meaning with its provenance and
epistemic classes, authoritative only through the existing admission owners.
...
There are two loops, not an automatic feedback write:

- **Computational:** E_t → execution → E_t+1 → subsequent execution. Lower
  describes initialization/reconstruction; internal updates need not recompile
  W_t or emit semantic events at every step.
- **Semantic:** explicit model/human proposal P_t or an observed external
  consequence → typed normalization → evidence/authority/policy/admission →
  admitted Transition → S_t+1. E_t+1 alone cannot enter this loop.
...
DeepSeek → Qwen may invalidate E_t, never the Case. Recompile the currently
required W_t from qualified S_t under current intent, scope, relevance and
budget. This does not promise identical hidden activations, stochastic output
or preservation of unadmitted computation. Computational state cannot be the
sole copy of a durable admitted Case fact.
...
| Computational-state amnesia | OPEN | Discard E_t through a qualified provider/operator boundary, preserve S_t and reconstruct required W_t/E_t for a bounded task. Loss of unadmitted computation or efficiency is permitted, loss of admitted Case meaning is not. Existing continuation-loss controls do not prove every future state mode. |
...
Test required big-picture retention, irrelevant-experience rejection,
supersession, task switching, long histories and correct semantic proposals.
